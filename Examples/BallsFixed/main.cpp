...
void Init()
{
	...
	sprintf(log_buff,"Creating threads...");log_file.Write(log_buff);
	{
		threads[0].offset=0;
	...
	sprintf(log_buff,"Render setup...");log_file.Write(log_buff);
	render->Set.VSync(false);

	render->Set.ClearColor(0,0,0);
	float f=powf(2,ball_pos_draw_precision);
	...
	sprintf(log_buff,"VBuffers creation...");log_file.Write(log_buff);
...
{	
	...
	{
		char buf[1000];
		sprintf(&buf[0],"HumMan Balls: %d %7.1f FPS Frame: %.3f ms  Phys: %.3f  Draw: %.3f ",
			balls_count,
			time.GetFPS(),
			time.GetTick()*1000,
			phys_time*1000,
	...
	render->BeginScene();
	{
		...
		TStreamsDesc streams;
		{
			...
			streams.AddStream(TStream::Vertex,TDataType::Short,2,pos_buff);
			//render->Blend.Func("dc*(1-sa)+sc*sa");
			render->Set.Projection(ortho_m);
...
int WINAPI WinMain (HINSTANCE hInstance,
					...
					int iCmdShow)
{   
	sprintf(log_buff,"Window creation...");log_file.Write(log_buff);
	MSG msg;
	...
	sprintf(log_buff,"Render creation...");log_file.Write(log_buff);
