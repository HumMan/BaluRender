...
TVectorList<TBVolume<float,3>> volumes;
...
void Init()
{
	for(int i=0;i<1;i++)
		for(int k=0;k<1;k++)
			for(int t=0;t<1;t++)
				volumes.Push(new TAABB<float,3>(TVec3(i*4,k*4,t*4),TVec3(1,1,1)));

	for(int i=0;i<1;i++)
		for(int k=0;k<1;k++)
			for(int t=0;t<1;t++)
				volumes.Push(new TSphere<float,3>(TVec3(i*4,k*4,t*4+6),1));
	...
	for(int i=0;i<1;i++)
		for(int k=0;k<1;k++)
			for(int t=0;t<1;t++)
			{
				...
				v0=v0.Cross(v1);
				v0.Normalize();
				volumes.Push(new TOBB<float,3>(TVec3(i*8,k*8,t*8+15),TMatrix3(v0,v1,v0.Cross(v1).GetNormalized()),TAABB<float,3>(TVec3(0),TVec3(1))));
			}
			...
			for(int i=0;i<5;i++)
				for(int k=0;k<1;k++)
					for(int t=0;t<1;t++)
					{
						...
						v1.MakeRandom();
						v1=v0+v1*3;
						volumes.Push(new TCapsule<float,3>(v0,v1,2));
			...
			{
				TVector<TVec3> vertices;
				TVector<unsigned int> indices;
				for(int i=0;i<=volumes.GetHigh();i++)
				{
					volumes[i]->DrawTriangles(vertices,indices);
				}
				prim_index_count=indices.GetCount();

				prim_vertex=render->VertexBuffer.Create(TVBType::Array,vertices.GetCount()*sizeof(TVec3));
				prim_index=render->VertexBuffer.Create(TVBType::Index,indices.GetCount()*sizeof(unsigned int));
				render->VertexBuffer.SubData(prim_vertex,0,vertices.GetCount()*sizeof(TVec3),&vertices[0]);
				render->VertexBuffer.SubData(prim_index,0,indices.GetCount()*sizeof(unsigned int),&indices[0]);
			}
			{
				TVector<TVec3> vertices;
				for(int i=0;i<=volumes.GetHigh();i++)
				...
				prim_lines_vertex_count=vertices.GetCount();
...
{	
	...
	{
		char buf[1000];
		sprintf(buf,"1 - Nearest 2 - Billenear    %7.1f FPS",time.GetFPS());
			...
			if(true)
			{
				...
				for(float i=-size;i<=size;i+=step)
				{
					for(float j=-size;j<=size;j+=step)
					{
						...
						for(int k=0;k<=volumes.GetHigh();k++)
						{
							float t,t0,t1,t2,t3;
							TVec3 n,n0,n1;
