...
struct TGlyphPacker
{
	...
	{
		TVec2i pos,size;
		TVec2 tex_coords[4]; // обход пр€моугольника CCW ???
...
#elif PLATFORM==PLATFORM_LINUX // не проверено - в документации по этому поводу как-то кор€во написано
...
TTexFontId TBaluRender::TTexFont::Create()
{
	...
	wchar_t text[]=L"_ абвгдеЄжзиклмноЌќѕ–—“”‘’÷„ЎўЁёя _abcdefg__яяя__??????,?????????dsf;jдвыоа";
	int len=wcslen(text);
